$(function () {
    (function () {
        if ($('#carouselSlider').length) {
            var swiper = new Swiper('#carouselSlider', {
                ...
                navigation: {
                    nextEl: '[data-carousel-ctrl="next"]',
                    prevEl: '[data-carousel-ctrl="prev"]'
                },
                breakpoints: {
                    701: {
                        coverflowEffect: {
    ...
    let designBlockSlider = null;
    const mediaSize = 990;
    /*Инициальзируем слайдер*/
    function designBlockSliderInit() {
        if (!designBlockSlider) {
            designBlockSlider = new Swiper('.design-block__slider', {
                slidesPerView: 1,
                slidesPerGroup: 1,
                spaceBetween: 30,
                autoplay: {
    ...
    function designBlockSliderDestroy() {
        if (designBlockSlider) {
            designBlockSlider.destroy();
            designBlockSlider = null;
        }
    }
    $(window).on('load resize', function () {
        ...
        if (windowWidth <= mediaSize) {
            ...
            designBlockSliderInit();
        } else {
            // Уничтожить слайдер если он был инициализирован
            designBlockSliderDestroy();
        }
    });
    ...
    function mainSliderInit() {
        if (!mainSlider) {
            mainSlider = new Swiper('.main-swiper', {
                spaceBetween: 30,
                loop: false,
                autoplay: {
                    delay: 3000,
